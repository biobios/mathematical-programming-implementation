...
ABCycle_ptr create_AB_cycle(std::vector<size_t>& finding_path,
                        size_t end_index,
                        ...
                        Environment& env)
{
    ...
    auto AB_cycle_ptr = env.object_pools.any_size_vector_pool.acquire_unique();
...
std::vector<ABCycle_ptr> find_AB_cycles(size_t needs,
...
{
    using namespace std;
    const size_t city_count = env.tsp.city_count;
    start_timer("find_AB_cycles_other_processes");
    ...
    auto cities_having_2_edges_ptr = env.object_pools.LRIS_pool.acquire_unique();
    auto cities_having_just_1_edge_ptr = env.object_pools.LRIS_pool.acquire_unique();
    ...
    auto parent1_copy_ptr = env.object_pools.doubly_linked_list_pool.acquire_unique();
    auto parent2_copy_ptr = env.object_pools.doubly_linked_list_pool.acquire_unique();
    ...
    auto visited_ptr = env.object_pools.any_size_vector_pool.acquire_unique();
    auto first_visited_ptr = env.object_pools.vector_of_tsp_size_pool.acquire_unique();
    ...
    end_timer("find_AB_cycles_other_processes");
    start_timer("find_AB_cycles_2_edges");
    ...
    while (cities_having_2_edges.size() > 0) {
        ...
        while (true) {  // ABサイクルを一つ見つけるまでループ
            ...
            if (current_edge_count == 1) {
                ...
                if (current_city == visited.front()) {
                    // ABサイクル構成処理
                    auto cycle = create_AB_cycle(visited, 0, cities_having_2_edges, cities_having_just_1_edge, env);
                    if (cycle->size() > 2) {
                ...
                if (current_city == visited.front()) {
                    ...
                    auto cycle = create_AB_cycle(visited, first_visited[current_city], cities_having_2_edges, cities_having_just_1_edge, env);
                    if (cycle->size() > 2) {
                        AB_cycles.emplace_back(std::move(cycle));
                    }
                    break;
        ...
        if (AB_cycles.size() >= needs) {
            end_timer("find_AB_cycles_2_edges");
            return AB_cycles; // 必要な数のABサイクルが見つかった
        }
    }
    
    end_timer("find_AB_cycles_2_edges");
    start_timer("find_AB_cycles_1_edge");
    ...
    while (cities_having_just_1_edge.size() > 0) {
        ...
        if (AB_cycles.size() >= needs) {
            end_timer("find_AB_cycles_1_edge");
            return AB_cycles; // 必要な数のABサイクルが見つかった
        }
    }
...
std::vector<Child> edge_assembly_crossover(const Individual& parent1, const Individual& parent2, size_t children_size,
    ...
    switch (env.eax_type) {
        case eax::EAXType::Rand:
            AB_cycles = find_AB_cycles(numeric_limits<size_t>::max(), parent1, parent2, rng, env);
            break;
        case eax::EAXType::N_AB:
            AB_cycles = find_AB_cycles(env.N_parameter * children_size, parent1, parent2, rng, env);
            break;
        default:
