...
namespace eax {
class ParentReferenceMerger {
public:
    ParentReferenceMerger(ObjectPools& object_pools)
    ...
    void operator()(IntermediateIndividual& working_individual,
                    ...
                    const std::vector<std::reference_wrapper<const Individual>>& reference_parents) {
        ...
        while (subtour_list.sub_tour_count() > 1) {
            ...
            for (size_t i = 1; i <= min_sub_tour_size; ++i) {
                ...
                for (size_t j = 0; j < reference_parents.size(); ++j) {
                    const Individual& parent_individual = reference_parents[j];
                    for (size_t parent_connected_city : parent_individual[current_city]) {
                        if (in_min_sub_tour[parent_connected_city])
                            continue;

                        for (size_t k = 0; k < 2; ++k) {
                            size_t connected_to_current_city = elem_of_min_sub_tour[i - 1 + 2 * k];
                            for (size_t l = 0; l < 2; ++l) {
                                size_t connected_to_parent_city = working_individual[parent_connected_city][l];

                                distance_type cost = - adjacency_matrix[current_city][connected_to_current_city] - adjacency_matrix[parent_connected_city][connected_to_parent_city]
                                                    + adjacency_matrix[current_city][parent_connected_city] + adjacency_matrix[connected_to_current_city][connected_to_parent_city];
                                if (cost < min_cost) {
                                    min_cost = cost;
