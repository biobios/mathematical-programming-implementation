...
std::pair<mpi::genetic_algorithm::TerminationReason, std::vector<Individual>> execute_ga(
    ...
    using Adaptive_N_AB = eax::EAX_normal<eax::N_AB_e_set_assembler_builder, eax::AdaptiveRangeMerger>;
    ...
    auto crossover_func = [&eax_n_ab, &eax_block2](const Individual& parent1, const Individual& parent2,
        ...
        switch (context.eax_type) {
            case eax::EAXType::One_AB:
                return eax_n_ab(parent1, parent2, env.num_children, env.tsp, context.random_gen, 1, std::forward_as_tuple(context.edge_counter, env.merge_range));
    ...
    struct {
        ...
        void update_individual_and_edge_counts(vector<Individual>& population, Context& context) {
            for (auto& individual : population) {
                ...
                context.entropy += eax::calc_delta_entropy(delta, context.edge_counter, context.env.population_size);
