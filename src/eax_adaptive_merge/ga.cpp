...
std::pair<mpi::genetic_algorithm::TerminationReason, std::vector<Individual>> execute_ga(
    ...
    // 交叉関数
    Adaptive_N_AB eax_n_ab(object_pools);
    Adaptive_Rand eax_rand(object_pools);
    Adaptive_UNIFORM eax_uniform(object_pools);
    auto crossover_func = [&eax_n_ab, &eax_rand, &eax_uniform](const Individual& parent1, const Individual& parent2,
        ...
        struct {
            ...
            auto operator()(const eax::EAX_Rand_tag&) {
                return eax_rand(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, {}, std::forward_as_tuple(context.edge_counter, context.env.merge_range));
            }

            auto operator()(const eax::EAX_n_AB_tag& n_ab) {
                return eax_n_ab(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, n_ab.get_n(), std::forward_as_tuple(context.edge_counter, context.env.merge_range));
            }

            auto operator()(const eax::EAX_UNIFORM_tag& uniform) {
                return eax_uniform(parent1, parent2, context.env.num_children, context.env.tsp, context.random_gen, uniform.get_ratio(), std::forward_as_tuple(context.edge_counter, context.env.merge_range));
            }
